...
function set_player_destroy_build_table($player) {
	...
	while($row = mysqli_fetch_array($query, MYSQLI_ASSOC)){
		$output .= '<tr><td><img src="images/icons/'.$row['blockID'].'.png" width="16px" height="16px" />&nbsp;&nbsp;<a class="ajax-link" href="index.php?mode=material-stats&material='.$row['blockID'].'"  >'.translate(''.$row['blockID'].'').':</a></td>';	
...
function set_material_destroy_table($material, $search) {
	...
	while($row = mysqli_fetch_array($query, MYSQLI_NUM)) {		
		...
		$output .= '<tr>';
		$output .= '<td style="width:250px;">'.$image.'&nbsp;&nbsp;<a class="ajax-link" href="index.php?mode=show-player&user='.$row[0].'">'.$row[0].':</a></td>';	
