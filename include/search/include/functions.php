...
function search_check_user($user) {
	...
	if($check = mysqli_fetch_array(mysqli_query($link, "SELECT `player` FROM `".WS_CONFIG_STATS."` WHERE `player`='".$user."' GROUP BY `player`"), MYSQLI_NUM)) {
		...
		return true;
	} elseif($check = mysqli_fetch_array(mysqli_query($link, "SELECT `username` FROM `".WS_CONFIG_ECONOMY."` WHERE `username`='".$user."' GROUP BY `username`"), MYSQLI_NUM)) {
		return true;
		...
		return true;
	} elseif($check = mysqli_fetch_array(mysqli_query($link, "SELECT `account` FROM `".WS_CONFIG_ECONOMY."` WHERE `account`='".$user."' GROUP BY `account`"), MYSQLI_NUM)) {
		return true;
